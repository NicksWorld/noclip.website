...
export class ChromeShader extends DeviceProgram {
    public static a_Position = 0;
    public static a_Color = 1;
    public static a_Uv = 2;
    public static a_Normal = 3;
...
layout(location = ${ChromeShader.a_Position}) in vec3 a_Position;
layout(location = ${ChromeShader.a_Color}) in vec4 a_Color;
layout(location = ${ChromeShader.a_Uv}) in vec2 a_Uv;
layout(location = ${ChromeShader.a_Normal}) in vec3 a_Normal;
...
void main() {
    ...
    vec3 norm = normalize(mat3(UnpackMatrix(u_WorldFromLocal)) * a_Normal);

    // TODO: Calculate proper envmap reflection projections
    v_TexCoord = norm.zy;
}
`;
